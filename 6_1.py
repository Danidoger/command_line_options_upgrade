...
def main():
    ...
    parser.add_argument("-n", "--new_arc", default="new",required=True, help="Имя нового архива")
    parser.add_argument("-th", "--third_fl", default="03",required=True, help="Имя третьего файла")
    new_arc_name = (parser.parse_args(sys.argv[1:])).new_arc
    if not (new_arc_name.endswith('.zip')):
        new_arc_name += ".zip"
    filename_3 = (parser.parse_args(sys.argv[1:])).third_fl
